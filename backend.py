...
def load_augmentation_class():
  #Loads the user augmentation class. Similar in spirit to django.contrib.auth.load_backend
  try:
...
def check_auth(self, username, password):
   #Implementors should return a boolean value which determines whether the given username and password pair is valid.
...
#Authentication backend that allows any user to login as long as they have a username. 
#The users will be added to the 'default_user_group'.
...
class PamBackend(DesktopBackendBase):
  #Authentication backend that uses PAM to authenticate logins. The first user to login will become the superuser.
